...
export function assembleLanguage(donorInfo, honorInfo) {
  ...
  const taxParagraph =
    donorInfo.taxParagraph === "Yes"
      ? `Please let this note serve as your receipt for a fully tax-deductible contribution of  ${donorInfo.donationAmount} 
  ...
  const standardDonorObj = {
    ...
    templateName:
      donorInfo.templateType === "New donor" ? "newdonor" : "recurringdonor",
  };
  ...
  const honorObj = () => {
    if (honorInfo.honorForm === "Yes") {
      ...
      const introSentence =
        donorInfo.templateType === "New donor"
          ? newDonorIntroSentence
          : recurringDonorIntroSentence;

      const actionSentence =
        honorInfo.honoringOrMemory === "In honor of"
          ? honorSentence
          : inMemoryOfSentence;
  ...
  function getEmailSubjectStr(donorInfo) {
    // return the way the donor name should be appear in the email header to the honoree
    let finalStr = "";

    let arrOfNames = donorInfo.donorNames;

    arrOfNames = Array.isArray(arrOfNames)
      ? arrOfNames
      : generateArrOfObjs(arrOfNames); //add objects to array where each object is its own element
  ...
  const honoreeObj = {
    //its going to look confusing but Im going to keep these where possible and add them into the node mailer templates on the server side
    ...
    emailSubject: `${getEmailSubjectStr(
      donorInfo
    )} has dedicated a donation to you`,
  };
  ...
  async function determineRelevantTemplates() {
    ...
    if (!honorerObj) {
      ...
      const nodeMailerResult = await sendEmailToServer([
        honoreeObj,
        honorerObj,
      ]);
