...
function App() {
  ...
  function getHonoreeVizStrs(obj) {
    // this function gets the strings needed for the honoree email from the visualizer to pass eventually to assembleLanguage file
            ...
            <div className={resultMessage !== null ? "opaqueduringpopup" : ""}>
              ...
              <div className="form-visualizer-grid">
                  ...
                  <div>
                    {formPage === 1 ? (
                      <div className="form-wrapper">
                        {" "}
                        <label className="template-field-name">
                          {" "}
                          Template Type:{" "}
                        </label>
                        <Dropdown
                          prefill={userChoices.templateType}
                          cssClass="long-dropdown"
                          options={["New donor", "Recurring donor"]}
                          updateUserChoice={updateUserChoice}
                          updateStep={updateStep}
                          updateKey="templateType"
                        />
                        <PresentForm
                          templateType={userChoices.templateType}
                          updateUserChoice={updateUserChoice}
                          userChoices={userChoices}
                          incompleteFields={incompleteFields}
                          recentPageChange={recentPageChange}
                          updatePageChangeState={updatePageChangeState}
                        />{" "}
                      </div>
                    ) : (
                      <PresentForm
                        templateType="Honoring"
                        updateStep={updateStep}
                        userChoices={userChoices}
                        getHonorState={getHonorState}
                        honoreeVizStrs={honoreeVizStrs}
                        getMsgStatustToRoot={getMsgStatustToRoot}
                      />
                    )}
                  </div>

                  {userChoices.templateType ===
                  "Choose Template Type" ? null : (
                    <NextStepBtn
                      userChoices={userChoices}
                      reportIncompleteFields={reportIncompleteFields}
                      goTotNextPreviousForm={goTotNextPreviousForm}
                      page={formPage}
                      updateStep={updateStep}
                    />
                  )}
                ...
                <Visualizer
                  ...
                  updateUserChoice={updateUserChoice}
                  getHonoreeVizStrs={getHonoreeVizStrs}
                />
              </div>
            </div>
          </div>
