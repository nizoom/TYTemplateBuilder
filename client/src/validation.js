...
export function donationFormValidation(state) {
  ...
  const futureDate = moment().add(2, "years").format("YYYY-MM-DD");

  if (moment(momentifiedDate._i, "YYYY-MM-DD").isValid()) {
    if (
      moment(momentifiedDate._i).isBefore("1990-01-01") ||
      moment(momentifiedDate._i).isAfter(futureDate)
    ) {
  ...
  const donorNames = Array.isArray(shallowCopyOfState.donorNames)
    ? shallowCopyOfState.donorNames
    : Object.values(shallowCopyOfState.donorNames);
  //get array of name validation and see if any elements are invalid
  const testedNames = validateDonorNames(donorNames);

  const invalidNameTrigger = testedNames.find(
    (testedName) => typeof testedName === "object"
  );

  //a return of undefined from the find method means that the element being filtered for was not found

  if (invalidNameTrigger !== undefined) {
    validityTracker.push({ namesStatus: testedNames });
  } else {
    validityTracker.push({ namesStatus: "valid" });
  }
  ...
  function validateDonorNames(namesArr) {
    ...
    if (namesArr.length < 1) {
      namesValidityTracker.push(
        { donorFirstName: false },
        { donorLastName: false }
      );
    } else {
      namesArr.forEach((nameObj) => {
        ...
        if (checkFirstName && checkLastName) {
          ...
          namesValidityTracker.push({
            donorFirstName: checkFirstName,
            donorLastName: checkLastName,
          });
        }
      });
    }
...
export function honorFormValidation(state) {
  ...
  if (shallowCopyOfState.honoringOrMemory === "in Honor or in Memory") {
  ...
  if (
    recipientNameObj.donorFirstName === "" ||
    recipientNameObj.donorLastName === ""
  ) {
    //if either are blank then return an object indicating the invalid name fields

    const recipientFirstNameStatus =
      recipientNameObj.donorFirstName === "" ? false : true;
    const recipientLastNameStatus =
      recipientNameObj.donorLastName === "" ? false : true;
    validityTracker.push({
      recipient: {
  ...
  if (shallowCopyOfState.honoreeName.length < 1) {
    ...
    if (
      honoreeNameObj.donorFirstName === "" ||
      honoreeNameObj.donorLastName === ""
    ) {
      //if either are blank then return an object indicating the invalid name fields
      const honoreeFirstNameStatus =
        honoreeNameObj.donorFirstName === "" ? false : true;
      const honoreeLastNameStatus =
        honoreeFirstNameStatus === true ? false : true;
      validityTracker.push({
        honoreeName: {
